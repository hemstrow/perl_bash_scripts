...
#Creates and outputs a full matrix of pairwise LD in Dprime or rsq according to argument. First in list is rsq, second is Dprime, third is a table of proximity and rsq, dprime, if asked for.
#input data: each row is a single snp, each column is an individual. Genotypes are in the "0000" format, as given by format_snps option 4.
#The position column needs to be named "position", the linkage group column "group", and the snp column "snp"
LD_full_pairwise <- function(data, num_start, prox_table = FALSE, matrix_out = TRUE) {
  i <- 1
  require(dplyr)
  #print(min(data$position))
  data$position <- as.numeric(data$position)
  data <- arrange(data, group, position)
  group <- data[i,"group"]
  rsq_name <- paste0(group, "_rsq")
  Dprime_name <- paste0(group, "_Dprime")
  if (matrix_out == TRUE){
    out_rsq <- data.frame()
    out_Dprime <- data.frame()
  }
  if (prox_table == TRUE){
    Nl <- nrow(data)
    out_prox <- as.data.frame(matrix(NA, nrow = ((Nl - 1)*Nl)/2, ncol = 7))
    colnames(out_prox) <- c("group", "proximity", "rsq", "Dprime", "pval", "p1", "p2")
    out_prox$group <- data$group[1]
  }
  if (prox_table != TRUE & matrix_out != TRUE) {
    stop("No output format specified.\n")
  }
  out_list <- list()
  count <- 1
  for(i in i:nrow(data)){
    g_list <- sort(unique(unlist(data[i,num_start:ncol(data)])))
    if(g_list[1] == "0000"){g_list <- g_list[2:length(g_list)]}
    num <- length(g_list)
    if(num == 1 | num > 3){
      #cat(num, "genotypes detected at snp:", data[i,1], ", Position:", data[i, "position"], ". This loci will be skipped.\n")
    ...
    for(j in (i+1):nrow(data)){
      if(j > nrow(data)){next} #stop the process from overflowing.
      if(count %% 500 == 0){cat("Progress:", paste0(round((count/(nrow(data)*(nrow(data) - 1)/2))*100, 1), "%."),
                                "Snps:", i, " and ", j, "\n")}
      #cat("\nj = ", j, "\n")
      p <- data[j,"position"]
      #cat(data[i,1], data[j,1], "\n")
      ...
      #print(out_rsq)
      key2 <- as.character(data[j, "position"])
      if (p<c){ #shouldn't happen if sorted properly
        next
      }
      else if (p == c){ #same
        Dprime <- NA
        ...
        #cat("\nPosition:", c, "vs. Position", p, "\n")
        g_list2 <- sort(unique(unlist(data[j,num_start:ncol(data)])))
        #print(g_list2)
        if(g_list2[1] == "0000"){g_list2 <- g_list2[2:length(g_list2)]}
        num <- length(g_list2)
        if(num == 1 | num > 3){
      ...
      if(matrix_out == TRUE){
        out_rsq[i,key2] <- rsq
        out_Dprime[i,key2] <- Dprime
      }
      if (prox_table == TRUE){
        prox <- abs(c - p)
        if (prox != 0){
          #cat("Printing to prox table:")
          #print(out_prox)
          out_prox[count,"proximity"] <- prox
          out_prox[count,"rsq"] <- rsq
          out_prox[count,"Dprime"] <- Dprime
          out_prox[count,"p1"] <- c
          out_prox[count,"p2"] <- p
          out_prox[count, "pval"] <- pval
          count <- count + 1
        }
      }
    }
  ...
  #need to figure out how to give two outputs.
  if(matrix_out == TRUE){
    out_list[[rsq_name]] <- out_rsq
    out_list[[Dprime_name]] <- out_Dprime
    if(prox_table == TRUE){
      out_prox <- out_prox[is.na(out_prox[,2]) == FALSE,]
      out_list[["prox"]] <- out_prox
    }
    return(out_list)
  }
  else if (prox_table == TRUE){
    out_prox <- out_prox[is.na(out_prox[,2]) == FALSE,]
    return(out_prox)
  }
}
...
Full_LD_w_groups <- function(data, num_start, prox_table = FALSE, matrix_out = TRUE, report = 1){
  ...
  for (i in 1:length(groups)){
    ...
    w_data <- LD_full_pairwise(data = w_data, num_start = num_start, prox_table = prox_table, matrix_out = matrix_out)
    if(prox_table == TRUE){
      if(matrix_out == TRUE){
...
format_snps_old <- function(data, ecs, output = 1, input_form = "NN", 
  ...
  if(output == 1){
    ...
    if(input_form != "0000"){
      stop("Only 0000 format accepted.")
    }
  }
  else if(output == 7){
  ...
  if(output == 1 | output == 5){
    ...
    if (is.list(pop) == FALSE){
      ...
      #loop through and do this for every row
      i <- 1
      for (i in i:nrow(data)){
        if(i %% 1000 == 0){cat("snp number: ", i, "\n")}
        t <- sum_row(data[i,(1 + ecs):ncol(data)]) #use sum_row to make a table of allele counts
        #print(t)
        t <- t[names(t) != miss] #removes missing data
        #print(t)
        w_df[i,"n_total"] <- sum(t) #add n_total to 
        w_df[i,"n_alleles"] <- length(t) #add n_alleles
        j <- 1
        for (j in j:length(t)) { #loop through table to add data for alleles to w_df
          w_df[i,paste0("ni", j)] <- t[j] #add data for this allele to w_df
        }
      }
      w_df[is.na(w_df)==TRUE] <- 0 #replace NAs with 0s.
    }
    else{
      require(plyr)
      ...
      for (i in i:nrow(data)){
        ...
        #cat("snp #:", i, "\n")
        start <- 0
        for (j in j:pop_count){ #loop through each pop, creating then appending to a table of allele counts
          #cat("Summing pop:", j, "\n")
          if(j == 1){ #for first pop only, initialize table
            t1 <- sum_row(data[i, (1 + ecs + start):(sum(ecs,pop_sizes[1:j]))]) #create table of allele counts
            t1 <- t(t1) #transposing table for later rbind.fill.matrix
            #cat("1st pop table:")
            #print(t1)
          }
          #for each pop other pop do the same, then rbind to table
          else{
            t1 <- rbind.fill.matrix(t1, t(sum_row(data[i, (1 + ecs + start):(sum(ecs,pop_sizes[1:j]))])))
            #cat("Table after", j, "nd/thpop:")
            #print(t1)
          }
          start <- start + pop_sizes[j] #increase the start variable.
        }
        #t1 is now the table of allele counts for pops, with NAs filling anything missing.
        t1 <- t1[,!(colnames(t1) %in% miss)] #remove missing data column
        #print(t1)
        #if(ncol(t1) > 2){ #kills function if more than two SNP options found. Could add support for more
        #  cat("More than two alleles detected at: SNP", i, ".\n\n Quitting...")
        #  stop
        #}
        itp <- seq(from = i, to = nrow(w_df), by = nrow(data))#create a list of the w_df indexes to add
                                                              #data to.
        
        #return(t1)
        #stop
        #cat("Data row:", i, "itp:", itp, "\n")
        #print(t1)
        ...
        k <- 1
        non_poly <- 0
        if(is.null(nrow(t1)) == TRUE){#checks to see if t1 is just a list rather than a matrix, as occurs
                                      #when the loci is non-polymorphic (which shouldn't really happen)
          #cat("Non-polymorphic loci detected!\n")
          non_poly <- 1 #sets check non-poly variable to 1 (TRUE)
        }
        remove_list <- numeric(0) #initialize list of rows to remove for option 5
        for(k in k:pop_count){#loop through to print data to correct locations.
          if(output == 1){
            if(non_poly == 1){ #prints 1 allele and the total number recorded if non-poly is true
              #cat("Calling non-poly for step 1.\n")
              w_df[itp[k], "n_total"] <- sum(t1[k], na.rm = TRUE)
              w_df[itp[k], "n_alleles"] <- 1
            }
            else{ #prints data if non-poly is false
              #cat("Calling poly for step 1.\n")
              w_df[itp[k], "n_total"] <- sum(t1[k,], na.rm = TRUE)
              w_df[itp[k], "n_alleles"] <- length(t1[k,])
            }
            j <- 1
            #print(k)
            #print(itp[k])
            if(non_poly == 1){#if non-poly, prints counts for each pop.
              #cat("Calling non-poly for step 2.\n")
              w_df[itp[k],paste0("ni", 1)] <- t1[k]
            }
            else{ #if non-poly is false, prints data.
              for (j in j:ncol(t1)) { #loop through this row of the table to add data for alleles to w_df
                #cat("Calling poly for step 2.\n")
                w_df[itp[k],paste0("ni", j)] <- t1[k,j] #add data for this allele to w_df
              }
            }
          }
            ...
            j <- 1
            if(non_poly == 1){ #prints nothing if non-poly, stores row names to remove.
              #cat("Calling non-poly for step 1.\n")
              remove_list <- c(remove_list, itp[k])
            }
            #print(k)
            #print(itp[k])
            else{ #if non-poly is false, prints data.
              for (j in j:ncol(t1)) { #loop through this row of the table to add data for alleles to w_df
                #cat("Calling poly for step 2.\n")
                w_df[itp[k],paste0("ni", j)] <- colnames(t1)[j] #add data for this allele to w_df
                w_df[itp[k],paste0("ni_count", j)] <- t1[k,j] #add data for this allele to w_df
              }
              w_df[itp[k],"n_total"] <- sum(t1[k,], na.rm = TRUE)
            }
          }
      ...
      if(output == 5){ #if doing output style 5...
        ...
        if(length(remove_list) != 0){#if there are rows to remove...
          w_df <- w_df[-remove_list,] #remove rows where the data was non-polymorphic
        }
      }
    }
  w_df[is.na(w_df)==TRUE] <- 0 #replace NAs with 0s.
  ...
  if (output == 3){
    require(plyr)
    ##Make a function to change snp data to structure format. Returns a list.
    strp <- function(x){
    ...
    samp_names <- colnames(data)[(ecs + 1):ncol(data)] #get sample names
    
    #prepare data and w_df
    if (is.na(n_samp) == FALSE){
      #get random sample IDs.
      snps_td <- sample(x = 1:nrow(data), n_samp, replace = FALSE)#get snp ids to do
      data <- as.data.frame(t(data[,(ecs + 1):ncol(data)])) #remove extra columns and transpose data
      w_df <- as.data.frame(matrix(data = NA, ncol = length(snps_td), nrow = (2*nrow(data)))) #create empty w_df
      colnames(w_df) <- snps_td #add snp numbers
      data <- data[,snps_td] #trim data file to only the snps to be analyzed. Data ready
    }
    else{
      data <- as.data.frame(t(data[,(ecs + 1):ncol(data)])) #transpose data after removing extra columns, data ready
      w_df <- as.data.frame(matrix(data = NA, ncol = ncol(data), nrow = (2*nrow(data)))) #create empty w_df
      colnames(w_df) <- colnames(data)#add snp numbers, w_df ready
    }
    
    #loop through data, convert, and add to w_df
    t_samp <- 1 #create variable to count which rows of w_df we are writing to
    for(j in 1:nrow(data)){ #loop through rows (samples)
      if(j %% 10 == 0){cat("sample number: ", j, "\n")}
      for(i in 1:ncol(data)){#loop through columns (snps)
        g <- strp(data[j,i]) #get and convert genotypes
        w_df[t_samp,i] <- g[1] #add allele one to w_df
        w_df[t_samp + 1,i] <- g[2] #add allele two to to w_df, one row below allele 1
      }
      t_samp <- t_samp + 2
    }
    names_df <- data.frame(s_names = samp_names, num = 1:length(samp_names))
    names_df <- rbind(names_df, names_df)
    names_df <- arrange(names_df, num)
    w_df <- cbind(names_df[,1],w_df) #adds sample names column back to w_df
  }
...
#Filters the data according to a set of fileters. The argument mDat is the dataformat for missing
#data, eg "0000" or "NN". Num_start is the number of the first column with data to filter. Only for bi-allelic data.
#The data must be in the format where rows are snps and columns are individuals.
#
#
#FILTERS:
...
#
#additional list return allf is a list of all snps to filter for any reason
#
#With the "apply_filt" argument as FALSE, a list will be returned with named sublist of the indices of snps to be filtered.
#The "check_all" argument determines if all filters should be checked if one is tripped or
#if the program should move to the next snp. Mostly for debugging
#
#
#Note, should really vectorize this where possible. Can probably vectorize a fair bit of it.
#old, non-vectorized version
filter_snps_old <- function(data, num_start, non_poly = FALSE, maf = FALSE, 
                        hf_hets = FALSE, min_ind = FALSE, apply_filt = TRUE, check_all = FALSE, mDat = "NN", maf_flt_pops = FALSE, pop = 1){
  non_poly_flt <- c()
  maf_flt <- c()
  hf_hets_flt <- c()
  mi_flt <- c()
  sum_row <- function(row){
    l <- c(substr(row,1,snp_form/2), substr(row,(snp_form/2) + 1, snp_form))
    t <- table(l)
    return(t) #returns a table with counts of the two alleles and then an N
  }
  half_miss <- substr(mDat, 1, nchar(mDat)/2)
  snp_form <- nchar(data[1, num_start])
  for(i in 1:nrow(data)){
    #cat("Row", i, "\n")
    if(i %% 1000 == 0){cat("snp number: ", i, "\n")}
    #print(snp_form)
    if(non_poly == TRUE){
      gs <- unique(unlist(data[i,num_start:ncol(data)]))
      gs <- gs[gs != mDat]
      #print(length(gs))
      if(length(gs) == 1 && substr(gs, 1, snp_form/2) == substr(gs,(snp_form/2) + 1, snp_form)){
        #cat("Non-poly at:", i, "\n")
        non_poly_flt <- c(non_poly_flt, i)
        if(check_all == FALSE){next}
      }
    }
    if(is.numeric(maf) == TRUE){ 
      if(maf_flt_pops == FALSE){ #if we only care about total maf...
        l1 <- c(lapply(X = data[i, num_start:ncol(data)], substr, start = 1, stop = snp_form/2), 
                lapply(X = data[i, num_start:ncol(data)], substr, start = (snp_form/2) + 1, stop = snp_form))
        t1 <- table(unlist(l1))
        t1 <- t1[names(t1) != half_miss]
        if(min(t1)/sum(t1) < maf){
          maf_flt <- c(maf_flt, i)
          #cat("maf_flt at:", i, "\n")
          if(check_all == FALSE){next}
        }
      }
      else{ #if we need to look at mafs by pop...
        
        require(plyr)
        pop_count <- length(unlist(pop[1]))
        pop_sizes <- unlist(pop[2])
        if(sum(unlist(pop[2])) != (ncol(data) - num_start + 1)){#checks to make sure that the pop sizes add up to agree 
          #with data
          cat("Supplied population numbers do not equal the number of supplied loci columns. Exiting.")
          stop()
        }
        ...
        for (j in j:pop_count){ #loop through each pop, creating then appending to a table of allele counts
          #cat("Summing pop:", j, "\n")
          if(j == 1){ #for first pop only, initialize table
            t1 <- sum_row(data[i, (num_start + start):(sum(num_start-1,pop_sizes[1:j]))]) #create table of allele counts
            t1 <- t(t1) #transposing table for later rbind.fill.matrix
            #cat("1st pop table:")
            #print(t1)
          }
          #for each pop other pop do the same, then rbind to table
            ...
            t1 <- rbind.fill.matrix(t1, t(sum_row(data[i, (num_start + start):(sum(num_start-1,pop_sizes[1:j]))])))
            #cat("Table after", j, "nd/thpop:")
            #print(t1)
          }
          start <- start + pop_sizes[j] #increase the start variable.
        }
        t1 <- t1[,colnames(t1) != half_miss] #remove the miss column
        t1[is.na(t1)] <- 0
        
        t1 <- cbind(t1, apply(t1, 1, min)/apply(t1, 1, sum)) #figure out the maf for each pop
        mafs <- t1[,3] #grab mafs only to remove NaNs
        mafs <- mafs[is.nan(mafs) == FALSE] #remove NaNs
        if(any(mafs > maf) == FALSE){
          maf_flt <- c(maf_flt, i)
          #cat("maf_flt at:", i, "\n")
          if(check_all == FALSE){next}
        }
    ...
    if(is.numeric(hf_hets) == TRUE){
      t1 <- table(unlist(data[i, num_start:ncol(data)]))
      t1 <- t1[names(t1) != mDat]
      if(any(substr(names(t1),1,snp_form/2) != substr(names(t1),(snp_form/2) + 1,snp_form)) == TRUE){
        #logical above says only do this if any hets are observed
        hc <- t1[substr(names(t1),1,snp_form/2) != substr(names(t1),(snp_form/2) + 1,snp_form)]
        hf <- hc/sum(t1)
        #cat(hc, sum(t1), hf, "\n")
        if(hf > hf_hets){
          hf_hets_flt <- c(hf_hets_flt, i)
          #cat("hets_flt at:", i, "\n")
          if(check_all == FALSE){next}
        }
      }
    }
    if(is.numeric(min_ind) == TRUE){
      t1 <- table(unlist(data[i, num_start:ncol(data)]))
      miss_ind <- t1[mDat]
      #cat(sum(t1), " ", miss_ind, " ", min_ind, "\n")
      if(is.na(miss_ind) == FALSE){
        if(sum(t1) - miss_ind < min_ind){
         mi_flt <- c(mi_flt, i)
         #cat("mi_flt at:", i, "\n")
         if(check_all == FALSE){next}
        }
  ...
  out_list <- list()
  if(non_poly == TRUE){
    out_list$npf <- non_poly_flt
  }
  if(is.numeric(maf) == TRUE){
    out_list$maff <- maf_flt
  }
  if(is.numeric(hf_hets) == TRUE){
    out_list$hfhf <- hf_hets_flt
  }
  if(is.numeric(min_ind) == TRUE){
    out_list$mif <- mi_flt
  }
  out_list$allf <- unique(unlist(out_list))
  if(apply_filt == FALSE){return(out_list)}
  else if (apply_filt == TRUE){
    data <- data[-out_list$all,]
    return(data)
  }
  else{
    warning("apply_flt must be TRUE or FALSE")
    stop()
    }
}
...
Full_LD_g_par <- function(data, num_start, num_cores, prox_table = FALSE, matrix_out = TRUE){
  if (prox_table == FALSE & matrix_out == FALSE){
    stop("Please specify output format.")
  }
  require(doParallel)
  ...
  output <- foreach(i = 1:length(groups), .export = 'LD_full_pairwise', .packages = "dplyr", .inorder = FALSE,
                    .options.snow = opts) %dopar% {
    w_data <- data[data$group == groups[i],]
...
format_snps <- function(data, ecs, output = 1, input_form = "NN", 
  ...
  if(output == 1 | output == 5){
    ...
    if (is.list(pop) == FALSE){
      ...
      if(any(rowSums(pals) != 2)){stop("More or less than two alleles detected at some loci.\n")}
