...
COLR.HUMB.2dsf = read.table("~/longfin/pop_gen/demographic_modeling_sfs/COLR.HUMB.2dsfs",header=FALSE)
COLR.HUMB.2dsf = as.numeric(COLR.HUMB.2dsf[1,])
dim(COLR.HUMB.2dsf) = c(13,17)
...
COLR.nr_ind = (nrow(COLR.HUMB.2dsf)-1)/2
HUMB.nr_ind = (ncol(COLR.HUMB.2dsf)-1)/2
# on the other hand the number of chromosomes can be retrieved as
COLR.nr_chrom = nrow(COLR.HUMB.2dsf)-1
HUMB.nr_chrom = ncol(COLR.HUMB.2dsf)-1
...
nr_snps = as.numeric(nr_sites-COLR.HUMB.2dsf[1,1]-COLR.HUMB.2dsf[(2*COLR.nr_ind+1), (2*HUMB.nr_ind+1)])
...
mu = 2e-8
#ref_pop_size <- 68000
min_theta = 4*Nef_min*mu*gen_time
max_theta = 4*Nef_max*mu*gen_time
min_M12 = 0
max_M12 = 100
min_M21 = 0
max_M21 = 100
...
cat("", file="~/longfin/pop_gen/demographic_modeling_sfs/results/Theta_M_distance.txt")
...
for (i in 1:nr_simul) # run this "for loop" nr_simul times
{
  ...
  theta_random_pop1 = runif(1, min = min_theta, max = max_theta)
  theta_random_pop2 = runif(1, min = min_theta, max = max_theta)
  ...
  Nef_random_pop1 = theta_random_pop1/(4*mu*gen_time)
  Nef_random_pop2 = theta_random_pop2/(4*mu*gen_time)
	# record the sampled value of divergence time in a file
	cat(Nef_random_pop1, "\t", Nef_random_pop2, "\t", M12_random, "\t", M21_random, "\t", file="~/longfin/pop_gen/demographic_modeling_sfs/results/Theta_M_distance.txt", append=TRUE)
	cat("\t", Nef_random_pop1, "\t", Nef_random_pop2, "\t", M12_random, "\t", M21_random)
	...
	ms.command <- paste(ms_dir, "28", nr_snps, "-s 1 -I 2 12 16 -n 1", theta_random_pop1," -n 2", theta_random_pop2," -m 1 2", M12_random," -m 2 1 ", M21_random," > ms.txt")
	#ms 15 1000 -t 2.0 -I 3 10 4 1 5.0 -m 1 2 10.0 -m 2 1 9.0
	# run ms
	system(ms.command)

	# read ms output file and compute the 2D-SFS
	simulated.2dsfs<-fromMStoSFSwith1site("ms.txt", nr_snps, COLR.nr_chrom, HUMB.nr_chrom)
  ...
  #	plot2DSFS(simulated.sfs, xlab="Polar", ylab="Brown", main="Simulated 2D-SFS")
	...
	# store it in a file
	cat(eucl_dist, "\n", file="~/longfin/pop_gen/demographic_modeling_sfs/results/Theta_M_distance.txt", append=T)
