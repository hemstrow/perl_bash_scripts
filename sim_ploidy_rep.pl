...
if($#ARGV == 5 | $#ARGV == 6){
	...
	$r = $ARGV[3]; #r: number of reps
	$p = $ARGV[4]; #p: ploidy
	$s = $ARGV[5]; #s: selfing, 0 = no, 1 = yes.
	if($#ARGV = 6){$os = $ARGV[6];} #os: output style. Empty for ms, 1 for only times and number of mutations, any other number for both.
}
else{die("Usage: sim_ploidy.pl	n_gene_copies	popsize	mutation_rate	number_reps	ploidy	selfing	optional_output_styles\n");}
...
print("ms $init_copies $r -t $theta\n\n\n");
